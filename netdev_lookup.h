...
inline std::map<std::string, std::string> get_device_descriptions() {
...
inline const std::map<std::string, std::string>& get_cached_device_descriptions() {
    static const std::map<std::string, std::string> cached = get_device_descriptions();
    return cached;
}

inline DeviceMapping match_iface_pcap(pcap_if_t* iface) {
